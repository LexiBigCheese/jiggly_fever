...
pub trait JigglyBoard {
    ...
    fn cols_mut(
        &mut self,
    ) -> impl IntoIterator<Item = impl IntoIterator<Item = impl MutSlime<Self::Loc>>>;
    ...
    fn run_physics(&mut self, dt: f32, physprop: &PhysicsProperties) -> bool {
        ...
        for col in self.cols_mut().into_iter() {
            let mut jiggle_offset = 0.0;
            for (y, mut slime) in col.into_iter().enumerate() {
                slime.modify_slime(
                    |SlimePropsIn {
                         state,
                         y_bottom,
                        ...
                        match state {
                            ...
                            Falling { velocity } => {
                                ...
                                if y_bottom <= y as f32 {
                                    ...
                                    let clamped_vel =
                                        velocity.mul(1.0 / 9.0).add(1.0).clamp(1.0, 2.0);
